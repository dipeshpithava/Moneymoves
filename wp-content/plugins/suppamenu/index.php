...
$suppa_settings = array (
	...
	'page_title'	=> 'Suppamenu Pro Settings' ,	// page_title
	'menu_title'	=> 'Suppamenu Pro' ,			// menu_title
	...
	public function __construct( $project_settings = array() ){
		...
		if( is_admin() && $thumbs = get_option('suppa_thumbs_sizes') ) {
			foreach ( $thumbs as $type => $siz ){
				...
				$resi->wp_actions();
				$resi = new ctf_resize_thumbnails( $siz['mega'][0], $siz['mega'][1] );
				$resi->wp_actions();
			}
		}
		...
		?>
				...
				<div class="codetemp_pages_container fl">
					...
					<div class="codetemp_page" id="codetemp_page_2_10">
						<?php require_once('standard/include/style-megaLinksTwo.php') ?>
					</div>
