...
(function( $ ){
   ...
   suppaMenuOB = {
       ...
       *
       */
      stickyMenu : function ( $menu, config ){

        var $rwd_width   = config.rwd_start_width,
        $window_width    = jQuery(window).width();

        // if the sticky enable, add the wrapper
        $menu.wrap('<div class="suppa-sticky-holder" style="height:' + $menu.height() + 'px !important;"></div>');

        // if this device width bigger than the responsive start width
        // and the scroll menu enable
        if( $window_width > $rwd_width
            && config.scroll_enable == 'on' ){

            var topOff = ( $menu.length) ? $menu.offset().top : 0 ;
            ...
            jQuery(window).on( 'scroll', function(){
                ...
                if( winTop > topOff ){
                  $menu.addClass('suppa-sticky');
                  $menu.find('.logo_top_center, .logo_top_left, .logo_top_right').hide();
                }
                else
                {
                  $menu.removeClass('suppa-sticky');
                  $menu.find('.suppa_menu_logo.logo_top_center, .suppa_menu_logo.logo_top_left, .suppa_menu_logo.logo_top_right').show();
                }
        ...
        // if responsive start width > window width
        // and sticky is enable on mobile
        if( $rwd_width >= $window_width
            && config.scroll_enable_mob == 'on' ){

          var topOff = $menu.offset().top;
          ...
          jQuery(window).on( 'scroll', function(){
              ...
              {
                $menu.removeClass('suppa-sticky');
                $menu.find('.suppa_rwd_logo.logo_top_center, .suppa_rwd_logo.logo_top_left, .suppa_rwd_logo.logo_top_right').show();
              }
          });
        }
      },
  ...
  $.fn.suppamenu = function( options ){
      ...
      /** Adjust Links Style Two , Links Container Width **/
      suppaMenuOB.adjustLinksStyleTwo(  $menu_wrap, config  );
      ...
      /** Scroll Menu **/
      suppaMenuOB.stickyMenu( $menu_wrap, config );
