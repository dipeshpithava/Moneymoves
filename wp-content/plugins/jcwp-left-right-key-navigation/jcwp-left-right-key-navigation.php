    ...
    */
    ...
    function jcorglrkn_inclscript() {
        ...
        if(is_single()) {
            ...
            $next_post_thumbnail = get_the_post_thumbnail($next_url->ID, "thumbnail"); // Get thumbnail Dip
            $previous_post_thumbnail = get_the_post_thumbnail($previous_url->ID, "thumbnail"); // Get thumbnail Dip
         ...
         jQuery(document).ready(function(){
             if(jQuery(document).width() > <?php echo intval(get_option("jcorglrkn_minimum_screen_width")?intval(get_option("jcorglrkn_minimum_screen_width")):0)?>) {
                 jQuery().jcNextPrev({nextLink:"<?php echo $next_url?>",
                     ...
                     nextThumb:'<?=$next_post_thumbnail?>',
                     prevThumb:'<?=$previous_post_thumbnail?>'
