...
Class TP_Template {
	...
	if ( is_object($poll) ):
	    ...
	    if ( !isset($this->available[$poll->template->name]) ):
	    ...
	    // Set settings
	    if ( isset($this->available[$poll->template->name]->settings) ):
		$poll->template->settings = $this->available[$poll->template->name]->settings;
	    endif;
	    // Set presets
	    if ( isset($this->available[$poll->template->name]->presets) ):
		$poll->template->presets = $this->available[$poll->template->name]->presets;
	    endif;
	...
	$css = tp_minify_css(apply_tp_filters('tp_template_pre_get_css', $this->get_part('style.css')));
    ...
    {
	$url = esc_url(tp_get_template_url($match[1]));
	...
	$default_path = $this->available[$poll->template->name]->settings;
	$default_way = false;
	// Lets Drive!
	// Section
	 ...
	 * @param Selector
	 */
	$ignored = apply_tp_filters('tp_template_prefix_ignore', array( '@media' => $match[0] ), $match[0]);
	 ...
	 */
	return apply_tp_filters('tp_template_prefix_id', $id, $match[0]);
    }
