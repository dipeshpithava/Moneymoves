...
Class TP_Poll_Editor {
	...
	// TotalPoll
	wp_enqueue_script('tp-admin', TP_JS_ASSETS . "admin-main$minimized.js", array(), TP_VERSION);
	...
	// Save opened tabs
	$last_opened_tabs = wp_parse_args(isset($_GET['tp_opened_tabs']) ? $_GET['tp_opened_tabs'] : array(), array( 'shortcode' => '1', 'question' => '1' ));
	...
	foreach ( $sections as $section ):
	     ...
	     * @param Options
	     */
	    do_tp_action("tp_admin_editor_before_{$section}_section", $options);
	    /**
	     * Include section.
	     */
	    include_once( TP_PATH . "includes/admin/editor/$section.php" );
	    /**
	     * After section.
	     * 
	     * @since 2.0.0
	     * @action tp_admin_editor_after_{$section}
	     * @param Options
	     */
	    do_tp_action("tp_admin_editor_after_{$section}_section", $options);
	endforeach;
	/**
	 * After editor.
	 * 
    ...
    {
	return add_query_arg(array( 'tp_opened_tabs' => $_POST['tp_opened_tabs'] ), $location);
    }
