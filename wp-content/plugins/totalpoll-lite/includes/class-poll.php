...
Class TP_Poll {
	...
	// Register shortcode
	add_shortcode('total-poll', array( $this, 'shortcode' ));
    }
	...
	if ( is_object($post) && $post->post_type == 'poll' ):

	    // Excute shortcode earlier (before sending headers)
	    $tagnames = array_keys($shortcode_tags);
	    $tagregexp = join('|', array_map('preg_quote', $tagnames));

	    // TotalPoll shortcode regex
	    $shortcode_regex = str_replace($tagregexp, 'total-poll', get_shortcode_regex());
	...
	$content_file = '';
	if ( ( !isset($poll->skip_to_results) || $poll->skip_to_results === false ) && TotalPoll()->security->has_ability_to_vote() ):
