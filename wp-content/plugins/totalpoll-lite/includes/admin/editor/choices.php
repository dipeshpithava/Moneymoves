...
<div class="choices section <?php echo!isset($last_opened_tabs['choices']) ? 'collapsed' : '' ?>">
    <input type="checkbox" name="tp_opened_tabs[choices]" class="tab-state" <?php checked(isset($last_opened_tabs['choices']), true); ?>>
    <h3 class="title"><?php _e('Choices', TP_TD); ?></h3>
    <div class="content clearfix">

	<?php do_tp_action('tp_admin_editor_before_choices_content', $options); ?>
        <label>
            <input type="checkbox" name="tp_options[override_votes]" class="override-votes"><?php _e('Override votes', TP_TD); ?>
        </label>
        <hr>
        <ul class="choice-types">
            <li><button type="button" class="button" data-template="choice-text">Text</button></li>
            <li><button type="button" class="button" data-template="choice-image"><?php _e('Image', TP_TD);?></button></li>
            <li><button type="button" class="button" title="<?php _e('Available in Pro version.', TP_TD);?>" disabled=""><?php _e('Link', TP_TD);?></button></li>
            <li><button type="button" class="button" title="<?php _e('Available in Pro version.', TP_TD);?>" disabled=""><?php _e('Video', TP_TD);?></button></li>
            <li><button type="button" class="button" title="<?php _e('Available in Pro version.', TP_TD);?>" disabled=""><?php _e('HTML', TP_TD);?></button></li>
	    <li><button type="button" class="button" title="<?php _e('Available in Pro version.', TP_TD);?>" disabled=""><?php _e('Shortcode', TP_TD);?></button></li>
		...
		<?php foreach ( $options->choices as $index => $choice ): ?>
		    ...
		    <?php elseif ( $choice->type == 'image' ): ?>
	    		...
	    		<input type="text" placeholder="<?php _e('Votes', TP_TD); ?>" name="tp_options[choices][<?php echo $index; ?>][votes]" value="<?php echo esc_attr($choice->votes) ?>" class="votes-counter widefat">
	    		<input type="text" placeholder="<?php _e('Image', TP_TD); ?>" name="tp_options[choices][<?php echo $index; ?>][image]" value="<?php echo esc_attr($choice->image) ?>" class="widefat upload-holder">
	    		<input type="text" placeholder="<?php _e('Full Size URL', TP_TD); ?>" name="tp_options[choices][<?php echo $index; ?>][full]" value="<?php echo esc_attr($choice->full) ?>" class="widefat">
	    		<input type="text" placeholder="<?php _e('Label', TP_TD); ?>" name="tp_options[choices][<?php echo $index; ?>][label]" value="<?php echo esc_attr($choice->label) ?>" class="widefat">
			...
			<?php $registered = has_filter("tp_admin_editor_{$choice->type}_choice_fields") || has_filter("tp_admin_editor_{$choice->type}_choice_buttons"); ?>
	    		...
	    		<input type="hidden" name="tp_options[choices][<?php echo $index; ?>][type]" value="<?php echo $choice->type; ?>">
	    		<input type="text" placeholder="<?php _e('Votes', TP_TD); ?>" name="tp_options[choices][<?php echo $index; ?>][votes]" value="<?php echo esc_attr($choice->votes) ?>" class="votes-counter widefat">
			    <?php
			    if ( $registered ):
				...
				foreach ( $choice as $key => $value ):
					...
					foreach ( $value as $subkey => $subvalue ):
					    ?>
			    		<textarea name="tp_options[choices][<?php echo $index; ?>][<?php echo $key; ?>][<?php echo $subkey; ?>]"><?php echo esc_attr($subvalue); ?></textarea>
					    <?php
					endforeach;
				    // Single value
				    else:
					?>
					<textarea name="tp_options[choices][<?php echo $index; ?>][<?php echo $key; ?>]"><?php echo esc_attr($value); ?></textarea>
				    <?php
				    endif;
				endforeach;
			    endif;
