...
class Super_Post_Meta {
	...
	**/
	...
	function wp_count_comments_d( $post_id = 0 ) {
	global $wpdb;

	$count = $wpdb->get_results("SELECT id, post_title, comment_count FROM {$wpdb->prefix}posts WHERE post_type='post' ORDER BY comment_count DESC");
	//return $count;
	return 0;
	//$post_id = (int) $post_id;
	...
	**/
	function the_content( $content ) {
		...
		//$comments_count = $this->wp_count_comments_d(get_the_ID());
		//$comments_count = 0;
		//$meta = get_post_meta( $post_id, 'super_post', true );
		$comments_count = get_comments_number()>1?get_comments_number()." Comments":get_comments_number()." Comment";
		
		$comments_html = "<li><i class='fa fa-comments-o'></i> ".$comments_count."</li>";
		

		$utils_html = $option['display_post_views'] || $option['display_post_likes'] || $option['display_post_ratings'] ? "<ul id='sp-utility-{$post->ID}' class='sp-utility sp-utility-default'>$views_html$likes_html$rates_html$comments_html</ul>" : "";		
